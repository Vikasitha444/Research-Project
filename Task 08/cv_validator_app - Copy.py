...
'''
        ...
        criteria = [
            ...
            ("Degree Name", "නිවැරදි degree name එක තියෙනවද"),
...
<body>
    <div class="container">
        ...
        <div class="results-header">
            ...
            <div class="summary-stats">
                <div class="stat-box issues">
                    ...
                    <div class="stat-label">🔧 හදන්න ඕන දේවල්</div>
                </div>
                <div class="stat-box passed">
                    <div class="stat-number">{{ passed|length }}</div>
                    <div class="stat-label">✅ හරියට තියෙන දේවල්</div>
                </div>
            </div>
        </div>
        ...
        <div class="check-section">
            <div class="check-header issues-header">⚠️ හදන්න ඕන දේවල් ({{ issues|length }})</div>
...
def job_recommendations_page():
    ...
    # Build filter dropdown values from jobs list if available
    jobs = (rec or {}).get("jobs", []) if isinstance(rec, dict) else []
    for j in jobs:
        if j.get("location"):
    ...
    job_types = sorted(list(set(job_types)))

    # Apply filters (optional)
    if location_filter:
        jobs = [j for j in jobs if j.get("location") == location_filter]
    if job_type_filter:
