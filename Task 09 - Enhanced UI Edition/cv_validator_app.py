...
def init_db():
    ...
    try:
        cur = conn.cursor()
        # Keep only what you use (removed unused "scores" table).
        cur.execute(
            """
...
def validate_with_llm(pdf_path: str, validation_results: Dict[str, Any], bundle: Dict[str, Any], temperature: float = 0.1):
    try:
        ...
        # IMPORTANT: remove formatting check, rename to structure check
        query = f"""
...
def calculate_overall_score(results: Dict[str, Any], llm_std: float = 0.0):
    ...
    # (stability removed -> llm_std fixed 0.0; keep hook if you later restore)
    if llm_std >= 2.0:
        weights["llm"] = 0.05
...
# Routes (UPDATED)
# -----------------------------
from flask import session  # ✅ add this at the top of your file (once)

# ✅ add this once after app = Flask(__name__)
...
def index():
    if request.method == "POST":
        ...
        results = None
        recommendations = []  # ✅ this will be filled
        ...
        try:
            ...
            # ✅ JOB RECOMMENDATIONS (CALL YOUR FUNCTION!)
            ...
            # ✅ store for /job-recommendations page
        ...
        return render_template(
            ...
            job_recommendations=recommendations,  # ✅ IMPORTANT (use this name in template)
        )
...
def api_validate():
    ...
    try:
        ...
        # ✅ include job recommendations in API output too (optional but useful)
...
if __name__ == "__main__":
    print("\n" + "=" * 50)
    print("  Enhanced CV Validator for Sri Lankan IT Market (FINAL)")
