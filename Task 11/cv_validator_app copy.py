...
# ──────────────────────────────────────────────────────────────
# OCR Configuration - FINAL FIXED FOR D DRIVE
# ──────────────────────────────────────────────────────────────
import platform
import os
import pytesseract
...
try:
    if platform.system() == "Windows":
        # ඔබගේ D drive එකේ පවතින නිවැරදි path එක
        _TESS_PATH = r"D:\setup files\ocr\tesseract.exe"
        
        if os.path.isfile(_TESS_PATH):
            pytesseract.pytesseract.tesseract_cmd = _TESS_PATH
            pytesseract.get_tesseract_version()
            _OCR_AVAILABLE = True
            print(f"\n[SUCCESS] Tesseract OCR is enabled from: {_TESS_PATH}")
        else:
            _OCR_AVAILABLE = False
            print(f"\n[WARNING] Tesseract NOT FOUND at: {_TESS_PATH}")
            print("Please check if tesseract.exe is actually inside D:\\setup files\\ocr")
    else:
        # For Linux/Mac
        pytesseract.get_tesseract_version()
        _OCR_AVAILABLE = True
...
TECH_KEYWORD_VARIANTS: Dict[str, List[str]] = {
    ...
    # Additional tools commonly appearing in LLM-generated job descriptions
    "Appium":           [r"\bappium\b"],
    "Maven":            [r"\bmaven\b"],
    "Gradle":           [r"\bgradle\b"],
    "TestNG":           [r"\btestng\b"],
...
# ──────────────────────────────────────────────────────────────
# Title → skill inference map
# ──────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────
# Title normalisation patterns (ordered longest-match first)
# Maps a raw job title → a stable role_key used as the aggregation
# bucket when building the data-driven skill map.
# ──────────────────────────────────────────────────────────────
TITLE_ROLE_PATTERNS: List[Tuple[str, str]] = [
    # Multi-word patterns first (longest-match priority)
...
def normalize_title_to_role(title: str) -> str:
    """Map a raw job title string to a stable role_key bucket."""
...
def _extract_skills_for_data_map(raw_skills: str, description: str = "") -> List[str]:
    """
    Extract skills from raw_skills / description for data-map building.

    Uses regex extraction first (handles 'ci/cd', 'microsoft azure' etc.
    correctly without splitting on '/'), then supplements with a
    comma/pipe-split pass for tools not yet in TECH_KEYWORD_VARIANTS
    (e.g. 'cucumber', 'adobe creative suite').
    """
    ...
    # Split on comma/pipe/bullet only — NOT '/' to preserve 'ci/cd'
...
# ──────────────────────────────────────────────────────────────
# Data-driven title → skill inference  (replaces hard-coded
# TITLE_SKILL_MAP).  Built once from the enriched CSV at startup
# and cached in _DATA_SKILL_MAP.
# ──────────────────────────────────────────────────────────────
_DATA_SKILL_MAP: Dict[str, List[str]] = {}   # populated by build_data_skill_map()
_DATA_SKILL_MAP_BUILT: bool = False

# Minimal static fallback for roles with zero scraped skill data
# (e.g. completely novel job types, no raw_skills, no description).
...
def build_data_skill_map(
    ...
    """
    Build a data-driven title→skills map from the enriched job DataFrame.

    Algorithm:
      1. For each job: normalize title → role_key
      2. Extract skills from raw_skills + description
      3. Aggregate per role_key using a frequency Counter
      4. Return top_n skills per role_key (ranked by frequency)

    This reflects ACTUAL Sri Lankan IT market requirements from real
    vacancy data rather than manually curated lists.
    """
    ...
    for _, row in df.iterrows():
        ...
        # Skip rows with no skill signal at all
    ...
    logger.info(
        f"Data-driven skill map built: {len(result)} role buckets "
        f"from {len(df)} jobs."
    )
    return result
...
def infer_skills_from_title(title: str) -> List[str]:
    """
    Return expected technical skills for a job title.

    Priority:
      1. Data-driven map (_DATA_SKILL_MAP) — built from actual scraped
         vacancy data; most accurate for Sri Lankan IT market.
      2. Static fallback (_STATIC_SKILL_FALLBACK) — minimal list for
         roles with no scraped skill data (novel/exotic job types).

    The data-driven map is populated by build_data_skill_map() which is
    called once inside build_combined_fields() when the DataFrame is ready.
    """
    global _DATA_SKILL_MAP_BUILT
    ...
    # Fallback: try parent role keys (e.g. 'qa_automation' -> 'qa')
    parent = role_key.split("_")[0]
    skills = _DATA_SKILL_MAP.get(parent)
    if skills:
        return skills

    # Final fallback: static minimal list
...
# Patterns to recognise soft skill lines from CV's own Soft Skills section
_CV_SECTION_SOFT_PATTERNS: Dict[str, List[str]] = {
    "communication":    [r"\bcommunicat\w*", r"\bverbal\b", r"\bwritten\s+skill",
                         r"\bpresentation", r"\bpublic\s+speaking", r"\binterpersonal"],
    "teamwork":         [r"\bteam[\s\-]?work", r"\bteam\s+player", r"\bcollaborat\w+",
                         r"\bco[\s\-]?operat\w+", r"\bgroup\s+work"],
    "problem_solving":  [r"\bproblem[\s\-]?solv\w+", r"\btroubleshoot\w+",
                         r"\bcritical\s+think\w+", r"\blogical\s+think\w+"],
    "leadership":       [r"\bleadership", r"\bteam\s+lead", r"\bmentor\w*", r"\bcoach\w*"],
    "time_management":  [r"\btime[\s\-]?management", r"\bdeadline", r"\bprioritiz\w+",
                         r"\bmulti[\s\-]?task\w+", r"\borganiz\w+"],
    "adaptability":     [r"\badaptab\w+", r"\bflexib\w+", r"\bquick\s+learn\w+",
                         r"\bfast\s+learn\w+", r"\bopen\s+to\s+change"],
    "creativity":       [r"\bcreat(?:ive|ivity)", r"\binnovat\w+", r"\bdesign\s+think\w+"],
    "attention_to_detail": [r"\battention\s+to\s+detail", r"\bdetail[\s\-]?oriented",
                             r"\baccurate\b", r"\bprecise\b", r"\bmeticulous"],
    "customer_focus":   [r"\bcustomer[\s\-]?(?:focus|service|centric)",
                         r"\bclient[\s\-]?(?:focus|facing|relation)"],
    "self_motivation":  [r"\bself[\s\-]?motivat\w+", r"\bproactiv\w+", r"\binitiativ\w+",
                         r"\bself[\s\-]?driven", r"\bindependent\s+work",
                         r"\bwork\w*\s+independently"],
    "analytical_thinking": [r"\banalytical\b", r"\bdata[\s\-]?driven", r"\bquantitative",
                             r"\bresearch\s+skill"],
    "conflict_resolution": [r"\bconflict\s+(?:resolution|management)", r"\bnegotiat\w+"],
    "mentoring":        [r"\bmentor\w*", r"\bcoach\w*", r"\bknowledge\s+shar\w+"],
    "presentation_skills": [r"\bpresentation\b", r"\bpublic\s+speaking", r"\bpitch\w*"],
    "project_management_mindset": [r"\bproject\s+manag\w+", r"\bscrum\s+master"],
}

# ──────────────────────────────────────────────────────────────
# Skill normalisation
# ──────────────────────────────────────────────────────────────
# FIX v4 BUG 2: Added multi-word LLM variant mappings so strings like
# "Microsoft Azure", "Agile Methodologies", "Google Cloud Platform"
# normalise to their canonical token instead of being dropped.
_SKILL_SYNONYMS: Dict[str, str] = {
    ...
    "restapi":                  "rest api",
    "rest":                     "rest api",
    # Multi-word LLM variants  (NEW in v4)
    ...
    "cloud computing":          "aws",      # generic → map to most common
    "microsoft azure":          "azure",
    "ms azure":                 "azure",
    "react.js":                 "react",
    "angular.js":               "angular",
...
def clean_skill_list(skills: Any) -> List[str]:
    """
    CV-side skill cleaner.
    Applies the KNOWN_TECH_SKILLS whitelist to avoid random words being
    treated as tech skills on the candidate's CV.
    Do NOT use this for job-side skills — use clean_job_skill_list() instead.
    """
    if not skills:
        return []
...
def clean_job_skill_list(skills: Any) -> List[str]:
    """
    FIX v4 BUG 1 — Job-side skill cleaner (NEW function).

    Permissive normalisation WITHOUT the KNOWN_TECH_SKILLS whitelist.
    LLM/scraper-generated skill strings like "Appium", "TestNG", "Maven",
    "Jenkins", "VMware", "Illustrator", "InDesign" are legitimate job
    requirements but are absent from TECH_KEYWORD_VARIANTS, so the strict
    clean_skill_list() would silently drop them.

    This function keeps every multi-char, non-generic-word token so that
    overlap scoring reflects the real job requirements.
    """
    if not skills:
        return []
    if isinstance(skills, str):
    ...
    for s in skills:
        ...
        if ns in _GENERIC_EMPLOYMENT_WORDS:
            continue
        # NO KNOWN_TECH_SKILLS filter here — allow all normalised tokens
        out.append(ns)
    return list(dict.fromkeys(out))
...
def infer_soft_skills_from_title(title: str) -> List[str]:
    """Return expected soft skills for a job title (deterministic, no API)."""
    if not title:
...
def _clean_skill_line(line: str) -> str:
    """Strip bullet symbols, numbers and extra whitespace from a CV line."""
    return re.sub(r"^[\s•\-\*◦▪→✓✔►\d\.]+\s*", "", line.strip()).strip()


def _split_skill_line(line: str) -> List[str]:
    """Split comma/semicolon/pipe separated skill line into parts."""
    parts = re.split(r"[,;|/]+", line)
    return [p.strip() for p in parts if p.strip() and len(p.strip()) > 1]


def _match_soft_skill_line(line: str) -> Optional[str]:
    """Map a cleaned single skill phrase → canonical name, or None."""
...
def _extract_from_line(raw_line: str) -> List[str]:
    """
    Given one raw line from the CV's soft skills section,
    return ALL canonical skill names found (handles comma-separated lines).
    """
...
def compute_soft_skill_match(
    ...
    """
    Score CV soft skills against what the job title expects.

    Standard set-based similarity:
      - job_expected: canonical soft skills inferred from the job title
      - score: Jaccard similarity = |CV ∩ Expected| / |CV ∪ Expected|
    """
...
def extract_pdf_bundle(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for page in doc:
            ...
            # ── OCR fallback: if text layer is empty/tiny, page is image-based ──
            if len(page_text) < 50 and _OCR_AVAILABLE:
                try:
                    # Render page to high-res image (300 DPI = good OCR accuracy)
                    mat  = pymupdf.Matrix(300 / 72, 300 / 72)   # 72 → 300 dpi
                    ...
                    if len(ocr_text.strip()) > len(page_text):
                        page_text = ocr_text
                        ocr_used  = True
                        logger.info(f"OCR used on page {page.number + 1} ({len(ocr_text)} chars extracted)")
                except Exception as ocr_err:
                    logger.warning(f"OCR failed on page {page.number + 1}: {ocr_err}")
...
def get_markdown_cached(pdf_path: str, bundle: Dict[str, Any]) -> str:
    """
    Return markdown representation of the PDF for the LLM prompt.

    If OCR was used (image-based PDF), pymupdf4llm.to_markdown() would
    return mostly empty content because it also relies on the text layer.
    In that case, use the OCR-extracted plain text from the bundle instead.
    """
    if bundle.get("markdown") is None:
        if bundle.get("ocr_used"):
            # OCR text is already extracted — wrap in minimal markdown
...
def check_formatting_quality(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        blocks = doc[0].get_text("blocks") if len(doc) > 0 else []
...
def check_skills_separation(pdf_path: str) -> Dict[str, Any]:
    """
    Detect Technical Skills and Soft Skills sections in the CV.
    """
    doc = None
    try:
        doc = pymupdf.open(pdf_path)
...
"""
        ...
        _LLM_SCORE_CACHE[file_hash] = result
        return result
...
def _fallback_body_text(soup: BeautifulSoup) -> str:
    """ප්‍රධාන selectors වලින් බැරි වුණොත් table/td ඇතුළේ ඇති text ලබා ගනී."""
    for tag in soup(["nav", "header", "footer", "script", "style", "aside", "form"]):
        tag.decompose()
    
    best = ""
    # TopJobs බොහෝ විට tabular structure එකක් පාවිච්චි කරන නිසා table/td පරීක්ෂා කරයි
    for block in soup.find_all(["div", "section", "article", "table", "td"]):
        txt = block.get_text(separator=" ", strip=True)
        # අනවශ්‍ය පොදු වැකි මග හැරීම
...
def preprocess_for_ocr(img_content):
    """OpenCV භාවිතයෙන් පින්තූරය OCR සඳහා සූදානම් කරයි."""
    ...
    # 2. Noise ඉවත් කිරීම (Median Blur)
    gray = cv2.medianBlur(gray, 3)
    
    # 3. Adaptive Thresholding (කළු-සුදු කිරීම - අකුරු මතු කර ගැනීමට)
    thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                   cv2.THRESH_BINARY, 11, 2)
    
    return _PILImage.fromarray(thresh)

def scrape_job_page(url: str, session_obj: requests.Session) -> Tuple[str, str]:
    if not url or not url.startswith("http"):
        return "", ""
    try:
        resp = session_obj.get(url, headers=SCRAPER_HEADERS, timeout=25)
        if resp.status_code != 200: return "", ""
        soup = BeautifulSoup(resp.text, "html.parser")
        
        raw_text = ""
        # 1. පින්තූරය සොයාගැනීම
        img_tag = soup.find('img', src=re.compile(r'JobAdvertismentServlet|vac-ad|flyer', re.I))
        
        if img_tag and _OCR_AVAILABLE:
            from urllib.parse import urljoin
            img_url = urljoin(url, img_tag['src'])
            time.sleep(1.5)
            img_resp = session_obj.get(img_url, timeout=20)
            
            if img_resp.status_code == 200:
                # --- ACCURACY BOOST: Image Pre-processing ---
                processed_img = preprocess_for_ocr(img_resp.content)
                raw_text = pytesseract.image_to_string(processed_img, lang='eng', config='--psm 6')

        # 2. පින්තූරය නැතිනම් හෝ අකුරු අඩු නම් Table එක බලන්න
        if len(raw_text.strip()) < 100:
            for table in soup.find_all('table'):
                txt = table.get_text(separator=" ", strip=True)
                if len(txt) > len(raw_text) and "Sign Up" not in txt[:50]:
                    raw_text = txt

        # 3. LLM Refinement (OCR එකෙන් ලැබුණු කුණු දත්ත පිරිසිදු කිරීම)
        # මෙය පාවිච්චි කරන්නේ raw_text එක පිරිසිදු කරලා හරියටම description & skills ගන්න
        if len(raw_text) > 100 and GROQ_API_KEY:
            client = OpenAI(api_key=GROQ_API_KEY, base_url="https://api.groq.com/openai/v1")
            refine_prompt = f"""
            I have some messy text from an OCR scan of a job ad. 
            Extract and summarize the core job description and technical skills.
            TEXT: {raw_text[:2000]}
            RETURN JSON ONLY: {{"description": "...", "skills": "..."}}
            """
            try:
                ...
                data = safe_json_loads(ai_resp.choices[0].message.content)
                desc = data.get("description", raw_text[:500])
                skills = data.get("skills", "")
                return desc, skills
            except:
                pass

        return raw_text.strip(), ", ".join(extract_skills_from_text(raw_text))
    except Exception as e:
        return "", ""
...
def enrich_csv_with_descriptions(
    ...
    def _needs_enrich(row: pd.Series) -> bool:
        ...
        return not desc or "please refer" in desc or len(desc) < 40

    to_enrich_mask = df.apply(_needs_enrich, axis=1)
    to_enrich      = df[to_enrich_mask]
    logger.info(f"Jobs to enrich: {len(to_enrich)}")
    ...
    for seq_num, (idx, row) in enumerate(to_enrich.iterrows()):
        ...
        try:
            ...
            if scraped_desc:
                ...
                logger.info(f"✓ scraped {title[:50]} → {str(df.at[idx, 'raw_skills'])[:60]}")
                continue
            ...
            if client is None:
                ...
                logger.info(f"✓ inferred {title[:50]} → {skills[:60]}")
                continue
            ...
            df.at[idx, "raw_skills"]  = skills
            cache[url] = (desc, skills)
            logger.info(f"✓ {title[:50]} → {skills[:60]}")

        except Exception as e:
            logger.warning(f"LLM failed for {title}: {e}")
            inferred = infer_skills_from_title(title)
            skills   = ", ".join(inferred)
            desc     = f"Role: {title}. Required skills: {skills}"
            df.at[idx, "description"] = desc
...
def build_combined_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Build data-driven skill map from this DataFrame (once per load) ──
    # Done here so the full dataset (including descriptions) is available.
    # infer_skills_from_title() will use this map going forward.
    if not _DATA_SKILL_MAP_BUILT:
        _DATA_SKILL_MAP       = build_data_skill_map(df)
        _DATA_SKILL_MAP_BUILT = True

    # ── Extract skills: regex on combined text, then supplement with
    #    data-driven inferred skills for jobs missing raw_skills ──
    ...
    # Always merge inferred skills as supplement (dedup via dict.fromkeys).
    # For jobs with good raw_skills, inferred adds little; for empty-skills
    # jobs the data-driven inferred list is the primary signal.
    ...
    # Use clean_job_skill_list (permissive) for job-side skills.
    df["job_skill_list"] = df["job_skill_list"].apply(clean_job_skill_list)
...
def build_user_skills_from_cv(results: Dict[str, Any], cv_text: str) -> List[str]:
    ...
    # CV side: use strict clean_skill_list to avoid noise
    return clean_skill_list(skills)


# ──────────────────────────────────────────────────────────────
# Main recommender  — updated with soft skill matching (v3)
#                     and job-skill pipeline fixes (v4)
# ──────────────────────────────────────────────────────────────
...
def get_job_recommendations(
    ...
    """
    Job recommender with soft skill matching and job-skill pipeline fixes.

    Scoring weights:
      50% TF-IDF text similarity
      30% Technical skill overlap  (uses clean_job_skill_list — permissive)
      10% Soft skill match         (CV section → job title inference)
    ...
    try:
        ...
        # ── Technical skills (CV side — strict filter) ────────────
        ...
        # ── Soft skills ───────────────────────────────────────────
        section_soft: List[str] = (
            (results.get("skills") or {}).get("extracted_soft_skills") or []
        )
        fulltext_soft: List[str] = _extract_soft_skills_fulltext(cv_text or "")
        cv_soft_skills_set: Set[str] = set(section_soft) | set(fulltext_soft)
        cv_soft_skills: List[str] = sorted(cv_soft_skills_set)
        ...
        # FIX v4 BUG 1 & 4: Use clean_job_skill_list (permissive),
        # return 0.3 neutral when no job skill data available.
        cv_set: Set[str] = set(user_skills)

        def _overlap_ratio(job_skills: Any) -> float:
            js = set(clean_job_skill_list(job_skills or []))
            if not js:
                # FIX v4 BUG 4: neutral score when job has no skill data,
                # so TF-IDF can still rank these jobs fairly.
                return 0.3
            return len(js & cv_set) / len(js)

        df["overlap"] = df["job_skill_list"].apply(_overlap_ratio)

        # ── Seniority alignment score ─────────────────────────────
        def _seniority_score(job_level: str) -> float:
            jl = (job_level or "").strip() or "intern_junior"
            if jl == cv_level:
                return 1.0
            allowed = _SENIORITY_ALLOWED.get(cv_level, set())
            return 0.7 if jl in allowed else 0.0
        ...
        # ── Soft skill score ──────────────────────────────────────
        def _soft_score(row: Any) -> float:
            info = compute_soft_skill_match(
                cv_soft_skills=cv_soft_skills,
                job_title=str(row.get("title", "")),
            )
            return info["score"]

        df["soft_score"] = df.apply(_soft_score, axis=1)
        ...
        for _, row in df.iterrows():
            job_title  = str(row.get("title", ""))
            # FIX v4 BUG 1: Use clean_job_skill_list for display too
...
def _extract_soft_skills_fulltext(cv_text: str) -> List[str]:
    """Fallback: scan full CV text for soft skill mentions when no section found."""
...
# ──────────────────────────────────────────────────────────────
def _run_analysis(filepath: str, filename: str, blind_mode: bool, location_filter: str, top_n: int = 5, seniority_mode: str = "filter") -> Dict[str, Any]:
    preview_info = save_preview_file(filepath, filename)
    file_hash    = sha256_file(filepath)
    bundle       = extract_pdf_bundle(filepath)

    # Surface OCR usage so frontend can show a notice
...
def index():
    ...
    try:
        results, recommendations = _run_analysis(
            filepath, filename, blind_mode, location_filter
        )

        rec_key = uuid.uuid4().hex
        _REC_CACHE[rec_key] = recommendations
...
# ──────────────────────────────────────────────────────────────
# Main Entry Point (Fixed Startup logic)
# ──────────────────────────────────────────────────────────────
if __name__ == "__main__":
    ...
    # Flask arguments සමග ගැටීම වැළැක්වීමට parse_known_args භාවිතා කරයි
    args, unknown = parser.parse_known_args()

    if args.enrich:
        print("\n" + "=" * 50)
        print("  STARTING DATA ENRICHMENT (OCR ENABLED)")
        print("=" * 50)
        ...
        # Flask server එක run වන්නේ මෙහිදී පමණි
        print("\n" + "=" * 55)
        print("  Enhanced CV Validator — Sri Lankan IT Market")
