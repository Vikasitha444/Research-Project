...
# OCR support — optional, graceful fallback if pytesseract / Pillow not installed
try:
    import pytesseract
    from PIL import Image as _PILImage

    # Windows: Tesseract installer puts the binary here by default.
    # The installer at tesseract-ocr-w64-setup-5.5.0.20241111.exe installs to
    # C:\Program Files\Tesseract-OCR\tesseract.exe
    # Set this before any pytesseract call so it doesn't rely on PATH.
    ...
    # Verify tesseract is reachable before marking OCR as available
...
def scrape_job_page(url: str, session_obj: requests.Session) -> Tuple[str, str]:
    if not url or not url.startswith("http"):
        return "", ""
    try:
        resp = session_obj.get(url, headers=SCRAPER_HEADERS, timeout=REQUEST_TIMEOUT, allow_redirects=True)
        ...
        generic_phrases = ["sign up", "login", "expired vacancy", "recaptcha"]
        if any(p in description.lower() for p in generic_phrases):
            return "", ""
...
if __name__ == "__main__":
    ...
    if args.enrich:
        enrich_csv_with_descriptions(csv_path=args.csv, cache_path=args.cache, force_rescrape=args.force)
