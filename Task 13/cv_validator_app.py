...
def is_valid_cv(bundle: Dict[str, Any]) -> Tuple[bool, str]:
    text = bundle.get("text_lower", "")
    
    # CV ekaka thiyenna ona moolika wachana
    cv_keywords = [
        "education", "experience", "skills", "projects", 
        "contact", "objective", "summary", "qualification",
        "work history", "employment"
    ]
    
    # Wachana kiyak thiyenawada kiyala balanna (uda list eken 3k wath thiyenna oni)
    found_keywords = [word for word in cv_keywords if word in text]
    
    if len(found_keywords) < 3:
        return False, "This document doesn't look like a CV. Please upload a professional resume."
    
    # Text eka godak adu nam (blank pages wage nam)
...
def index():
    ...
    try:
        # 1. Bundle eka extract karagannawa check karanna kalin
        bundle = extract_pdf_bundle(filepath)
        
        # 2. CV ekakda kiyala identify karagannawa
        valid, error_msg = is_valid_cv(bundle)
        if not valid:
            _remove_file(filepath) # Invalid nam file eka delete karanawa
            return render_template("index.html", results=None, error=error_msg), 400

        # 3. CV eka valid nam analysis eka patan gannawa
        ...
        return render_template("index.html", results=None, error="An error occurred during analysis."), 500
    finally:
        # Meka thama wadagath block eka error eka nathi karanna
        gc.collect()
